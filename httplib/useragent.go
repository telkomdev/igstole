...
func randomUserAgent() string {
	...
	operatingSystems := []string{"Windows 3.1", "Windows 95", "Windows 98", "Windows 2000", "Windows NT", "Linux 2.4.22-10mdk", "FreeBSD",
	...
	locals := []string{"cs-CZ", "en-US", "sk-SK", "pt-BR", "sq_AL", "sq", "ar_DZ", "ar_BH", "ar_EG", "ar_IQ", "ar_JO",
	...
	return fmt.Sprintf("%s/%d.%d (%s %d.%d; %s;)", browsers[rand.Intn(len(browsers)-1)], rand.Intn(20-1)+1, rand.Intn(20), operatingSystems[rand.Intn(len(operatingSystems)-1)], rand.Intn(7-1)+1, rand.Intn(9), locals[rand.Intn(len(locals)-1)])
}
